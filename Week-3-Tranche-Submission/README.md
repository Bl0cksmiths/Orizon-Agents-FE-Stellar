...
**The two PDFs.** [`Proof-of-Deliverables.pdf`](./Proof-of-Deliverables.pdf) (30 pages) captions every piece of evidence in this bundle: the story and hardening pull requests, both repositories' merged-PR lists for the week, QA's commits, four live API probes, the two testnet transactions, the two blocking defects, the defect register, and the shipped dispute interface. [`Technical-Documentation-and-Demo-Evidence.pdf`](./Technical-Documentation-and-Demo-Evidence.pdf) (23 pages) is the illustrated walkthrough: the dispute path as the buyer sees it, the money path's safeguards with a file-and-ADR citation for each, how to verify the deployment and the chain yourself, independent QA, and why D3 has no evidence run yet. Their sources are in [`screenshots/`](./screenshots/) and [`technical-documentation/`](./technical-documentation/), each with a manifest naming every image's origin.

Two kinds of screenshot appear in those documents and they are never mixed. Pages captured from the live testnet deployment or from public GitHub and Stellar Expert pages say so and carry their source URL. Pages showing the dispute interface are a **local run against the test suite's fixtures**, labelled in the page header, because the dispute path cannot be reached on the deployment yet — every transaction hash in them exists on no ledger. There is no Week-3 demo recording: the D3 walkthrough is blocked on the same defect, and the technical documentation says so on its last page rather than substituting anything.
